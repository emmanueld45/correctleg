...
if (!isset($_GET['itemisfor']) or $_GET['itemisfor'] == "") {
    $admin->goToPage("./", "invalid_filter");
}

if (!isset($_GET['itemprice']) or $_GET['itemprice'] == "") {
    $admin->goToPage("./", "invalid_filter");
}


$item_is_for = mysqli_real_escape_string($db->conn, $_GET['itemisfor']);
...
$item_type = "temporary";
$item_size = "temporary";
$item_price = mysqli_real_escape_string($db->conn, $_GET['itemprice']);

if ($item_price == "1000 - 5000" or $item_price == "5000 - 10000" or $item_price == "10000 - 20000" or $item_price == "20000 - 30000" or $item_price == "30000+") {
} else {
    $admin->goToPage("./", "invalid_filter");
}
...
// price range calculation start
if ($item_price == "1000 - 5000") {
    $from = 1000;
    $to = 5000;
} else if ($item_price == "5000 - 10000") {
    $from = 5000;
    $to = 10000;
} else if ($item_price == "10000 - 20000") {
    $from = 10000;
    $to = 20000;
} else if ($item_price == "20000 - 30000") {
    $from = 20000;
    $to = 30000;
} else if ($item_price == "30000+") {
    $from = 30000;
    $to = 1000000;
}
    ...
    <div class="main-area">
        ...
        <div class="right">
            ...
            <div class="item-container">
                ...
                if (!isset($_SESSION[$item_is_for . $item_price . $item_type . $item_size . 'items'])) {
                    $_SESSION[$item_is_for . $item_price . $item_type . $item_size . 'items'] = [];
                    if ($item_is_for == "All") {
                        $result = $db->setQuery("SELECT * FROM product WHERE price>=$from AND price<=$to AND status='active' ORDER BY RAND();");
                    } else {
                        $result = $db->setQuery("SELECT * FROM product WHERE itemisfor='$item_is_for'  AND price>=$from AND price<=$to AND status='active' ORDER BY RAND();");
                    }
                    while ($row = mysqli_fetch_assoc($result)) {
                        $_SESSION[$item_is_for . $item_price . $item_type . $item_size . 'items'][count($_SESSION[$item_is_for . $item_price . $item_type . $item_size . 'items'])] = $row['uniqueid'];
                    }
                }
                $main_item_array =  $_SESSION[$item_is_for . $item_price . $item_type . $item_size . 'items'];
            ...
            <div style='width:100%;padding:10px;text-align:center;font-size:19px;color:grey;'>Other items you may like:</div>
                ...
                <?php
                if (count($main_item_array) == 0) {
                    $result1 = $db->setQuery("SELECT * FROM product WHERE itemisfor='$item_is_for' ORDER BY RAND() LIMIT 10;");
                ...
                <div class="pagination-container2">
                    ...
                    if ($page != 1) {
                        ...
                        echo '<a href="filter?itemisfor=' . $item_is_for . '&itemprice=' . $item_price . '&page=' . $prev_page_count . '" class="prev"><i class="fa fa-angle-left"></i> PREV PAGE</a>';
                    }
                    echo '<span class="page-count"><b>' . $page . '</b>/' . $number_of_pages . '</span>';
                    if ($page < $number_of_pages) {
                        $next_page_count = $page + 1;
                        echo '<a href="filter?itemisfor=' . $item_is_for . '&itemprice=' . $item_price . '&page=' . $next_page_count . '" class="next"> NEXT PAGE <i class="fa fa-angle-right"></i></a>';
                    }
                    ?>
                </div>
