    ...
    <section class="cart-wrapper">
        ...
        ?>
            <section class="cart-container">
                <?php
                if (isset($_GET['item_unavailable'])) {
                ...
                ?>
                    <div class="box">
                        ...
                        <div class="row2">
                            ...
                            <span item_id="<?php echo $item_id; ?>" size="<?php echo $item['item_size']; ?>" class="size">Size: <?php echo $item['item_size']; ?></span>
                        ...
                        <div class="row3">
                            <?php
                            if (isset($_SESSION['id']) and $customer->customerIsLoggedIn($_SESSION['id']) and $customer->haveSavedItem($_SESSION['id'], $item_id)) {
                                echo "<button item_id='$item_id' class='save-icon'><i class='fa fa-heart'></i></button>";
                            } else {
                                echo "<button item_id='$item_id' class='save-icon'><i class='fa fa-heart stroke-transparent'></i></button>";
                            }
                            ?>
...
<script>
    ...
    $(".plus").click(function() {
        ...
        var item_size = $(".size[item_id=" + item_id + "]").attr("size");
    ...
    $(".save-icon").click(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data == "itemsaved") {
                    $(".save-icon[item_id=" + item_id + "] i").removeClass("stroke-transparent");
                    show_site_alert("item saved")
                } else if (data == "itemalreadysaved") {
                    $(".save-icon[item_id=" + item_id + "] i").addClass("stroke-transparent");
                    show_site_alert("item unsaved!")
                } else if ("loginfirst") {
                    show_site_alert("Please login first to save item. <a href='customers/login?cart'>Click here</a>")
                }
            }
